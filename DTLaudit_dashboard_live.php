...
table{width:100%;table-layout:fixed;border-collapse:collapse;background:rgba(19,22,25,.75);border:1px solid var(--line)}col.project-col{width:180px}col.data-col{width:72px}th{color:var(--muted);text-align:left;font-size:11px;letter-spacing:.10em;text-transform:uppercase;padding:12px 7px;border-bottom:1px solid var(--line);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}td{padding:11px 7px;border-bottom:1px solid rgba(255,255,255,.055);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}tr:hover td{background:rgba(56,189,248,.06)}.project{font-weight:700}.yes{color:var(--yes);font-weight:700}.no{color:var(--no);font-weight:700}.warn{color:var(--warn);font-weight:700}.okrow td:first-child{border-left:3px solid var(--yes)}.workrow td:first-child{border-left:3px solid var(--warn)}.badrow td:first-child{border-left:3px solid var(--no)}.error{border:1px solid rgba(255,122,69,.55);background:rgba(255,122,69,.08);padding:16px;border-radius:8px;color:var(--no);white-space:pre-wrap}
</style>
</head>
<body>
<header>
...
function hasReadme(p){return (p.files||[]).some(e=>["readme.md","readme_fr.md","readme_en.md"].includes(String(e.path||"").toLowerCase()))}
...
function render(data){
 ...
 const rows=projects.slice().sort((a,b)=>a.name.localeCompare(b.name,"fr",{sensitivity:"base"})).map(p=>{
  ...
  return `<tr class="${cls}"><td class="project" title="${safe(p.name)}">${safe(p.name)}</td><td class="version" title="${safe(version)}">${safe(version)}</td><td>${yn(!!g.present)}</td><td class="github">${yn(hasGh)}</td><td class="visibility" title="${safe(vis)}">${safe(vis)}</td><td>${yn(hasReadme(p))}</td><td>${yn(!!d.manuel_ref_fr)}</td><td>${yn(!!d.guide_user_fr)}</td><td>${yn(!!d.ref_manual_en)}</td><td>${yn(!!d.user_guide_en)}</td><td>${yn(hasRel)}</td><td>${safe(g.branch||"-")}</td><td>${m?`<span class="warn">${m}</span>`:'<span class="yes">0</span>'}</td></tr>`;
 }).join("");
